...
//import std.typecons.Tuple : expand;
...
struct Computer
{
    ...
    void Compute_position() nothrow
    {
        ...
        StyleValueErrorMessage!( "position" );
    }
...
string Compute_Enum( ARGS... )( string pName, ARGS args )
{
    string result = "";
    ...
    static foreach( i; 0 .. ARGS.length )
    {
        static if ( ( i & 1 ) == 0 )
        {
            result ~=
                format!
                    q{
                        if ( style.%s.valueString == "%s" )
                        {
                            computed.%s = %s.%s;
                            return;
                        }
                    }
                    ( pName, args[ i ], pName, typeof( args[ i + 1 ] ).stringof, args[ i + 1 ] );
        }
    }
