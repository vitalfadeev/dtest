...
struct Drawer
{
    void DrawTree( HDC hdc, Element* element, RECT* limits, POINT* offset, RECT* margin ) nothrow
    {
        // Root
        DrawRoot( hdc, element, limits, offset, margin );
    ...
    pragma( inline )
    void DrawRoot( HDC hdc, Element* element, RECT* limits, POINT* offset, RECT* margin ) nothrow
    {
        if ( element.computed.displayBox == DisplayBox.CONTENTS )
        {
            ...
            {
                case DisplayOutside.INLINE   : Display_Outside_Inline( element, limits, offset, margin ); break;
                case DisplayOutside.BLOCK    : Display_Outside_Block ( element, limits, offset, margin ); break;
            ...
            Rectangle( 
                hdc, 
                element.computed.offsetLeft, 
                element.computed.offsetTop, 
                element.computed.offsetWidth, 
                element.computed.offsetHeight 
            );
...
*/
    ...
    void CalculateWidthAuto( Element* element, RECT* limits, int* width ) nothrow
    {
        ...
        //
        if ( element.firstElementChild !is null )
        {        
            for( auto c = element.firstElementChild; c !is null; c = c.nextElementSibling )
            {
    ...
    pragma( inline )
    void DrawChilds( HDC hdc, Element* element, RECT* limits, POINT* offset, RECT* margin ) nothrow
    {
        final switch ( element.computed.displayInside )
        {
            case DisplayInside.FLOW      : DisplayChilds_Flow( hdc, element, limits, offset, margin ); break;
            case DisplayInside.FLOW_ROOT : break;
            case DisplayInside.TABLE     : break;
            case DisplayInside.FLEX      : DisplayChilds_Flex( hdc, element, limits, offset, margin ); break;
    ...
    pragma( inline )
    void DisplayChilds_Flow( HDC hdc, Element* element, RECT* limits, POINT* offset, RECT* margin ) nothrow
    {
        if ( element.firstElementChild !is null )
        {        
            for( auto c = element.firstElementChild; c !is null; c = c.nextElementSibling )
            {
                DrawTree( hdc, c, limits, offset, margin );
            }
        }
    }
    ...
    pragma( inline )
    void DisplayChilds_Flex( HDC hdc, Element* element, RECT* limits, POINT* offset, RECT* margin ) nothrow
    {
        if ( element.firstElementChild !is null )
        {        
            for( auto c = element.firstElementChild; c !is null; c = c.nextElementSibling )
    ...
    pragma( inline )
    void PutElement( Element* element, int left, int top, POINT* offset, RECT* margin ) nothrow
    {
        ...
        offset.x = element.computed.rect.right;
        offset.y = element.computed.rect.top;
    ...
    pragma( inline )
    void Display_Outside_Inline( Element* element, RECT* limits, POINT* offset, RECT* margin ) nothrow
    {
        // Строный 

        final switch ( element.computed.position )
        {
            case Position.STATIC   : Display_Outside_Inline_Static( element, limits, offset, margin ); break;
            case Position.RELATIVE : break;
            case Position.ABSOLUTE : break;
            case Position.STICKY   : break;
            case Position.FIXED    : break;
        }
        ...
        // Wrap
        if ( limits.left + offset.x + elementWidth > limits.right )
        { 
            PutElement( 
                element, 
                limits.left + element.computed.marginLeft, 
                offset.y + element.computed.lineHeight, 
                offset,
                margin
            );
        }
        else // No-Wrap
        {
            PutElement( 
                element, 
                offset.x + max( margin.right, element.computed.marginLeft ), 
                offset.y, 
                offset,
                margin
            );
    ...
    pragma( inline )
    void Display_Outside_Inline_Static( Element* element, RECT* limits, POINT* offset, RECT* margin ) nothrow
    {
        int newRight;

        // No-Wrap
        if ( ElementInLimits( element, limits, &newRight ) )
        {
            element.computed.offsetLeft = offset.x + newRight;
        }
        else // Wrap
        {
            //
    ...
    pragma( inline )
    bool ElementInLimits( Element* element, RECT* limits, int* newRight ) nothrow
    {
        *newRight = element.computed.right;
        *newRight = element.computed.left + element.computed.width;

        if ( *newRight <= limits.right )
        {
            return true;
        }
    ...
    pragma( inline )
    void Display_Outside_Block( Element* element, RECT* limits, POINT* offset, RECT* margin ) nothrow
    {
        // Блочный
